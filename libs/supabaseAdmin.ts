...
await stripe.customers.update(customer,{name,phone,address})
  ...
  const manageSubscriptionStatusChange = async(subscriptionId:string,customerId:string,creactAction= false)=>{
    const {data:customerData,error:NocustomerError} =await 
    supabaseAdmin.from("customers").select("id").eq("stripe_customer_id",customerId).single()
    ...
    const subscriptionData:Database["public"]["Tables"]["subscriptions"]["Insert"]={
        ...
        if(creactAction && subscription.default_payment_method && uuid){
